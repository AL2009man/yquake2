...
static Vector3 TransformToLocalSpace(float yaw, float pitch, float roll) {
    // ---- Combine Inputs ----
    float yawSensitivity = 1.0f;       // Yaw sensitivity scaling
    float pitchSensitivity = 1.0f;    // Pitch sensitivity scaling
    float rollSensitivity = 1.0f;      // Roll sensitivity scaling
    float couplingFactor = 0.075f;     // Refined Yaw-Roll coupling

    // Adjust Roll and harmonize with Yaw
...
static Vector3 TransformToPlayerSpace(float yaw_input, float pitch_input, float roll_input, Vector3 gravNorm) {
    ...
    // ---- Adjust Inputs Using Sensitivity ----
    float yawSensitivity = 1.0f;    // Yaw sensitivity
    float pitchSensitivity = 1.0f; // Pitch sensitivity
    float rollSensitivity = 1.0f;   // Roll sensitivity

    float adjustedYaw = (yaw_input * yawSensitivity) * gravNorm.y + pitch_input * gravNorm.z;
    float adjustedPitch = pitch_input * pitchSensitivity; // Direct pitch adjustment
...
static Vector3 TransformToWorldSpace(float yaw_input, float pitch_input, float roll_input, Vector3 gravNorm) {
    ...
    Vector3 rawGyro = Vec3_New(
        pitch_input * pitchSensitivity,      // Adjust Pitch directly
        -yaw_input * yawSensitivity,         // Adjust Yaw alignment
        roll_input * rollSensitivity         // Adjust Roll alignment
    );
    ...
    Vector3 worldGyro = Vec3_New(
        -Vec3_Dot(rawGyro, gravNorm),  // Yaw alignment with gravity
        Vec3_Dot(rawGyro, pitchAxis), // Pitch alignment with gravity
